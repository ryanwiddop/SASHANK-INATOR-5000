...
func viewTeam(c *gin.Context) {
	...
	res := db.Limit(50).Preload("Results").Order("time desc").Find(&records, "team_id = ?", team.ID)
	if res.Error != nil {
		errorOutGraceful(c, res.Error)
		return
	}
...
func submitInject(c *gin.Context) {
	...
	if !team.IsAdmin() {
		...
		if ZeroTime.Add(newSubmission.Time.Sub(startTime)).After(inject.Closes) {
			c.HTML(http.StatusOK, "inject.html", pageData(c, "Injects", gin.H{"error": "inject is no longer accepting submissions", "inject": inject}))
			return
		}
		//if ZeroTime.Add(newSubmission.Time.Sub(startTime)).In(loc).After(inject.Closes.In(loc)) {
		//	    c.HTML(http.StatusOK, "inject.html", pageData(c, "Injects", gin.H{
		//		"error": inject.Time.Format("2006-01-02 15:04:05 MST") + " | " + inject.Due.Format("2006-01-02 15:04:05 MST") + " | " +  inject.Closes.Format("2006-01-02 15:04:05 MST") + " | " + ZeroTime.Add(newSubmission.Time.Sub(startTime)).In(loc).Format("2006-01-02 15:04:05 MST"),
		//		"inject": inject,
		//	     }))
		//}
